...
gbm_assay_with_metadata <- cbind(gbm_assay_with_metadata, meta_gbm$paper_IDH.status)
...
```{r}
df_data_mut=data.frame(group = colData(ddsSE_IDH)$paper_IDH.status, t(df_data)) #ja estava em cima mas pronto
df_data_mut = na.omit(df_data_mut)
df_data_mut$group
pie(table(df_data_mut$group))
```
```{r}
ind = sample(2, nrow(df_data_mut), replace = T, prob=c(0.7, 0.3))
ind
```

```{r}
train = df_data_mut[ind==1, ]
test = df_data_mut[ind==2, ]
dim(train)
dim(test)
table(train$group)
table(test$group)
...
```{r}
library(class)  #ver
knn_pred = knn(train[, 1:4], test[, 1:4], train$group)
```
...
pecc(test$species, nb_pred)
sum(nb_pred == test$group)/length(test$group)
```
```{r}
library(party)
yeah_ctree <- ctree(group ~., data=train)
print(yeah_ctree)
```
```{r}
