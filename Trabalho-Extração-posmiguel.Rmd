...
```{r}
rot <- pca$rotation #contribuição de cada gene para cada componente principal


genes_ordenados <- order(apply(abs(rot), 1, max), decreasing = TRUE)
genes_selecionados <- rownames(rot)[genes_ordenados[1:100]]
genes_selecionados     #variavel com 100 genes com coeficientes de correlação maior (dava para a 1ªparte do trabalho); da para fazer cluster por expressão
```
...
```{r}
dist_matrix <- dist(gbm_assay, method = "euclidean")
dist_matrix1 <- dist(gbm_assay, method = "manhattan")
dist_matrix2 <- dist(gbm_assay, method = "minkowski")

```

```{r}
hc <- hclust(dist_matrix, method = "complete")
hc1 <- hclust(dist_matrix1, method = "complete")
hc11 <- hclust(dist_matrix2, method = "complete")

hc2 <- hclust(dist_matrix, method = "single")
hc22 <- hclust(dist_matrix, method = "single")
hc222 <- hclust(dist_matrix, method = "single")

hc3 <- hclust(dist_matrix, method = "average")
hc33 <- hclust(dist_matrix, method = "average")
hc333 <- hclust(dist_matrix, method = "average")
```
...
```{r}
sil_scores <- numeric(10) #silhueta (demora muito correr)

for (k in 2:10) {
  kmeans_result <- kmeans(reduced_data, centers = k)
  sil_scores[k] <- silhouette(kmeans_result$cluster, dist(reduced_data))$avg.width
}

plot(2:10, sil_scores, type = "b", xlab = "Number of Clusters (k)", ylab = "Average Silhouette Width")
```
...
```{r}
gbm_assay_transposed <- t(gbm_assay)
gbm_assay_with_metadata <- cbind(gbm_assay, meta_gbm$paper_IDH.status)
colnames(gbm_assay_with_metadata)[176] = "idh_status"
gbm_assay_with_metadata
```


```{r}
ind = sample(2, nrow(gbm_assay_with_metadata[, 1:176]), replace=TRUE, prob=c(0.7, 0.3))  #nao sei porque nao assume a coluna demetadados
trainData = gbm_assay[ind==1,]
testData = gbm_assay[ind==2,]
```
...
```{r}
library(class)
knn_pred = knn(trainData[,], testData[,], trainData$idh.status)
knn_pred
```
...
=======
#library(genefilter)
>>>>>>> 2acb5b6b14a62aac2c74ca3d008685026d3b9650
...
<<<<<<< HEAD
```{r}
dis_man = dist(ddsse_idh_rank, method = "manhattan")
dis_man
```
```{r}
clustering_hier2 = hclust(dis_man)
plot(clustering_hier2)
```

```{r}
clustering_hier22 = hclust(dis_man, method = "single" )
plot(clustering_hier22)
```

```{r}
clustering_hier3 = hclust(dis_man, method = "average")
plot(clustering_hier3)
```

```{r}
#meter transposta? igual para tudo
=======
...
#k-means
```{r}
var_kmeans = kmeans(ddsse_idh_rank, centers = 2, nstart = 10000)   
var_kmeans
```
```{r}
table(var_kmeans$cluster)   #ver nomes
```
```{r}
plot(ddsse_idh_rank, col=var_kmeans$cluster)
```
#machine learning
```{r}
...
table(knn_pred, test$group)
pecc(knn_pred, test$group)
...
nb_pred
table(nb_pred, test$group)
pecc(test$group, nb_pred)
sum(nb_pred == test$group)/length(test$group)
```
## Decision Tree
...
```{r}
# NAO FUNCIONA 
modelo <- lm(train$group ~ ., data = train)

previsoes <- predict(modelo, newdata = test)

RMSE(previsoes, test$group)

```

```{r}
library(e1071)   #erros marados
...
iris.rf = randomForest(group ~ ., data=train, importance=TRUE)
pred.rf = predict(iris.rf, test)
pecc(pred.rf, test$group)
table(pred.rf, test$group)
```

```{r}
library(nnet)
nn = nnet(train$group,train[, 2:ncol(train)], size=5)
...
group_nn_cv = train(train$group~., data = res_IDH_best[,2:ncol(res_IDH_best)], method = "nnet", tuneLenght=10, trControl=cv.ctrl, maxit = 100,)
```
